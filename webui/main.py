...
WEB_JOBS.mkdir(parents=True, exist_ok=True)  # create on first run if missing
STATIC_DIR = Path(__file__).resolve().parent / "static"
SUPPORTED_EXTS = {".tif", ".tiff", ".png", ".jpg", ".jpeg"}

app = FastAPI(title="FHN ColonyNet iPad Postprocessing")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def _extract_green_mask(rgb: np.ndarray) -> np.ndarray:
    """Extract exact-green pixels (R=0, G=255, B=0) from CPSAM overlay PNGs."""
    mask = (rgb[:, :, 0] == 0) & (rgb[:, :, 1] == 255) & (rgb[:, :, 2] == 0)
    return (mask.astype(np.uint8) * 255)
    ...
    for i, r in enumerate(rows_in):
        ...
        try:
            if len(post_mask_paths) >= 2 and post_mask_paths[0].exists() and post_mask_paths[1].exists():
                ...
                algo_mask = None
            # Fallback for CPSAM: no post_mask files exist, but tif_paths contain
            # the CPSAM overlay with exact-green (R=0,G=255,B=0) colony pixels.
            if algo_mask is None:
                try:
                    if len(tif_paths) >= 2 and tif_paths[0].exists() and tif_paths[1].exists():
                        gm0 = _extract_green_mask(_read_rgb_anydepth(tif_paths[0]))
                        gm1 = _extract_green_mask(_read_rgb_anydepth(tif_paths[1]))
                        candidate = _stitch_mask_side_by_side(gm0, gm1, gap=12)
                    elif tif_paths and tif_paths[0].exists():
                        candidate = _extract_green_mask(_read_rgb_anydepth(tif_paths[0]))
                    else:
                        candidate = None
        ...
        rows.append({
            ...
            "algo_count": r.get("algo_count"),
            "cpsam_count": r.get("cpsam_count"),
